...
drop table temporary_data_free;
drop table temporary_data_solve;
...
drop table game_true_ending;
drop table game_bad_ending;

-- ** sequence **

drop sequence seq_member_logger;
...
drop sequence seq_temporary_data_postno;
drop sequence seq_temporary_solve_postno;

-- ** sequence **
create sequence seq_member_logger;

create sequence seq_board_free_postno;
create sequence seq_board_solve_postno;
create sequence seq_game_rank_playno;
create sequence seq_notice_noticeno;

create sequence seq_comment_free_commentno;
create sequence seq_comment_solve_commentno;
create sequence seq_comment_rank_commentno;

create sequence seq_temporary_data_postno;
create sequence seq_temporary_solve_postno;
...
create table member(
    ...
    useremail varchar2(30) unique not null, --이메일
...
create table adminList(
    adminid varchar2(15),
    adminvote number default 0,
    constraint fk_adminid foreign key(adminid) references member(userid)
);

insert into adminList values('admin', default);
insert into adminList values('hoho', default);
insert into adminList values('wewe', default);
...
insert into game_rank values(seq_game_rank_playno.nextval, 'wewe', 4646546, to_date('20190102', 'yyyymmdd'));
insert into game_rank values(seq_game_rank_playno.nextval, 'hoho', 654654654, to_date('20190102', 'yyyymmdd'));
insert into game_rank values(seq_game_rank_playno.nextval, 'sun123', 545454545, to_date('20190105', 'yyyymmdd'));
insert into game_rank values(seq_game_rank_playno.nextval, 'heyhey', 8789515, to_date('20190105', 'yyyymmdd'));
insert into game_rank values(seq_game_rank_playno.nextval, 'babo', 31352135, to_date('20190106', 'yyyymmdd'));
insert into game_rank values(seq_game_rank_playno.nextval, 'baboba', 96232323, to_date('20190107', 'yyyymmdd'));
insert into game_rank values(seq_game_rank_playno.nextval, 'chunjae', 56565655, to_date('20190109', 'yyyymmdd'));
insert into game_rank values(seq_game_rank_playno.nextval, 'mungmung', 54564463, to_date('20190110', 'yyyymmdd'));
insert into game_rank values(seq_game_rank_playno.nextval, 'tomato', 76564532, to_date('20190117', 'yyyymmdd'));
insert into game_rank values(seq_game_rank_playno.nextval, 'abc', 86564532, to_date('20190117', 'yyyymmdd'));
...
insert into game_prologue(no, content) values(6,'얼굴이 말이 아니네, 그런표정 짓고 있는 거 보니까.');
insert into game_prologue(no, content) values(7,'그래도 너 때문에 망가져사는 나에 비할건 못돼.');
insert into game_prologue(no, content) values(8,'물론 널 갈기갈기 찢어놓고 싶은 마음이 굴뚝같지만,');
insert into game_prologue(no, content) values(9,'그리고 난 너를 죽일만큼 그렇게 간도 크지못해.');
insert into game_prologue(no, content) values(10,'네가 이 버튼을 누르고 이 방을 나가면,');
insert into game_prologue(no, content) values(11,'조금 뒤엔 경찰차와 구급차가 올거야.');
insert into game_prologue(no, content) values(12,'그리고 그 사람들이 도착할 즈음엔 난 이 세상 사람이 아니겠지.');
insert into game_prologue(no, content) values(13,'방에 갇혀있는 동안 뭐라도 느꼈길 바래.');
insert into game_prologue(no, content) values(14,'내가 너한테..');
insert into game_prologue(no, content) values(15,'마지막으로..');
insert into game_prologue(no, content) values(16,'남길...말은');
...
insert into game_true_ending(no, content) values(13, '별 볼일 없는 내인생.. 대학으로 조금이라도 바꿔볼 수 있었어.');
insert into game_true_ending(no, content, audioname) values(14, '너 이새끼 내가 시험지 뒤에 종이 있는거 다봤다!', 'people.wav');
insert into game_true_ending(no, content, audioname) values(15, '야.. 그렇게 크게 말하면 사람들이 진짠줄 알거아냐!', 'people.wav');
...
insert into game_true_ending(no, content, filename) values(18, '난 가정형편이 어려워서 가끔 몰래 급식을 먹곤 했어.', 'past_lunch.jpg');
insert into game_true_ending(no, content) values(19, '그 탓에 미운털이 잔뜩 박혀있었지.');
insert into game_true_ending(no, content, filename) values(20, '내가 시험 전에 보려고 여기저기 가방에 넣어둔 종이쪽지들...', 'papers.jpg');
...
--상점 힌트 테이블 데이터 추가
insert into game_hint values(1, '물은 쓸모가 있습니다.');
insert into game_hint values(2, '넌 탈출할 수 없어');
insert into game_hint values(3, '화분은 망치로 깰 수 있습니다.');
insert into game_hint values(4, '두 개의 잠금장치를 모두 해제해야 합니다.');
...
insert into game_hint values(10, '열쇠는 총 두 개 입니다.');
insert into game_hint values(11, '형광등은 망치로 부술 수 없습니다.');
insert into game_hint values(12, '이 방을 나가면 선택을 해야합니다.');
insert into game_hint values(13, '금고 비밀번호는 3자리입니다.');
insert into game_hint values(14, '도어락은 부술 수 없습니다.');
insert into game_hint values(15, '3, 1');
...
--게시글 임시저장 (자유게시판)
create table temporary_data_free(
    datano number,
    datatitle varchar2(50),
    datawriter varchar2(15),
    datacontent varchar2(2000),
    dataoriginalfile varchar2(100),
    datarenamedfile varchar2(100),
    datadate date default sysdate,
    constraint pk_temporary_data_postno primary key(datano)
);

--게시글 임시저장(공략게시판)
create table temporary_data_solve(
    datano number,
    datatitle varchar2(50),
    datawriter varchar2(15),
    datacontent varchar2(2000),
    ...
    datarenamedfile varchar2(100),
    datadate date default sysdate,
    constraint pk_seq_temporary_solve_postno primary key(datano)
);
...
insert into game_object values(12, 'wire', default, default, 'used_safe', 'front', 3, 3, 'Y');
